  ...
  getSimilarArtists = async (artists) => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const data = await fetch(
        `https://ws.audioscrobbler.com/2.0/?method=artist.getsimilar&artist=${artists[i]}&api_key=${this.apiKey}&format=json&autocorrect[0|1]`
      );
      ...
      mappedSimilarArtists.push(
        parsed.similarartists.artist.map((artist) => {
          return artist.name;
        })
      );
    }
